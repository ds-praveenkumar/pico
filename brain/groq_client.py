"""OpenAI-compatible client for Groq inference endpoints."""
...
class GroqClient(BaseLLM):
    ...
    def client_init(self, **kwargs):
        if not self.api_key:
            raise ValueError("GROQ_API_KEY is required to initialize GroqClient")
        self.client = OpenAI(
            api_key=self.api_key,
            base_url=self.base_url,
            **kwargs,
        )
        logger.info(f"[bold cyan]Groq client initialized[/bold cyan]: provider={self.provider} model={self.model_name} base_url={self.base_url}")
