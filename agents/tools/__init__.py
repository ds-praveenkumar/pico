...
REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    "ego_lite_browse_use": {
        ...
        "description": "Drive the ego-lite browser on the current page. action='load' opens 'url' and snapshots it. action='click' clicks 'selector'. action='fill' types 'query' into 'selector'. action='select' picks the 'query' option in the dropdown 'selector'. action='press' sends the 'query' key (default Enter) to submit a form. action='claim' reclaims a tab handed off to the master (after they say they are done) and snapshots it. action='release' close the browser task space once the goal is done so the browser claim is freed for the next task. The page stays open between calls. For actions use the stable locators from the snapshot (loc=css:..., CSS, or text); snapshot refs like @5 only work in the very same call. Provide 'url' with the first load and keep it in later calls. When the page shows a CAPTCHA, OTP, login/OTP, or a required form the result includes 'need_human': the tool announces what the master must do and WAITS for them to hand control back, then submits the form itself (CAPTCHA, OTP, and required-form pages; login pages stay read-only) and returns the result page with 'resumed'/'submitted' — just report that result and never fill a CAPTCHA answer, OTP, or code yourself. When the result includes 'paused' (the wait timed out or an earlier hand-off is open), the tab is parked under the master's control: ask through 'ask_master' whether they are done, then resume with action='claim' and read the result page. Never end the task while a hand-off is unresolved.",
